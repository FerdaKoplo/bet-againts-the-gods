...
# Signal kustom yang akan kita kirim ke battle.gd
signal option_selected(button)

func _ready() -> void:
	# Loop semua anak (tombol) di dalam menu ini
	for button in get_children():
		if button is BaseButton:
			# Hubungkan sinyal 'pressed' milik tombol ke fungsi kita
			button.pressed.connect(_on_button_pressed.bind(button))

func _on_button_pressed(button: BaseButton):
	# Oper tombol yang ditekan ke siapa saja yang mendengarkan (battle.gd)
	option_selected.emit(button)

# Fungsi untuk memindahkan fokus cursor (dipanggil oleh battle.gd)
# Signal baru untuk perpindahan kursor
signal button_focused(button) 

func button_focus(index: int = 0):
	if get_child_count() > 0:
		var button = get_child(index)
		if button is BaseButton:
			button.grab_focus()
			# Kabari panah bahwa kita sedang menyorot tombol ini
